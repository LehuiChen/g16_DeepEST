...
if __package__ in (None, ""):
    ...
    from deepest_backend import DeePESTEvaluator, get_available_deepest_models
...
def _deepest_add_args(parser):
    parser.add_argument(
        "--variant",
        default="main",
        choices=("main", "t1x"),
        help="DeePEST published checkpoint variant (default: main 10-element model).",
    )
    parser.add_argument("--dtype", default="float64", help="Delta-MACE default dtype (float32 or float64)")
    parser.add_argument(
        "--delta-hessian-step",
        type=float,
        default=1.0e-3,
        help="Step (Angstrom) used when numerical delta Hessian fallback is needed.",
    )
    parser.add_argument(
        "--calc-opt",
        action="append",
        default=[],
        metavar="KEY=VALUE",
        help="Extra kwargs passed to the delta-MACE calculator (advanced).",
    )
...
def _deepest_make_evaluator(args):
    return DeePESTEvaluator(
        model=args.model,
        ...
        delta_hessian_step=args.delta_hessian_step,
        calc_kwargs=_parse_kv_list(args.calc_opt, "--calc-opt"),
    )
...
_BACKEND_CONFIG = {
    "uma": {
        ...
        "include_xtb_correction_args": True,
    },
    "orbmol": {
    ...
    "deepest": {
        "default_model": "auto",
        "add_args": _deepest_add_args,
        "make_evaluator": _deepest_make_evaluator,
        "available_models": get_available_deepest_models,
        "available_tasks": None,
        "include_xtb_correction_args": False,
    },
}
...
def run_backend(backend_key, plugin_name, argv):
    ...
    return run_g16_plugin(
        ...
        include_xtb_correction_args=cfg.get("include_xtb_correction_args", True),
    )
