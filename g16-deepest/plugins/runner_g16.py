...
def run_g16_plugin(
    ...
    parser.add_argument(
        "--xtb-cmd",
        default="xtb",
        help="xTB executable path/command (default: xtb).",
    )
    parser.add_argument(
        "--xtb-acc",
        type=float,
        default=0.2,
        help="xTB --acc value (default: 0.2).",
    )
    parser.add_argument(
        "--xtb-workdir",
        default="tmp",
    ...
    if include_xtb_correction_args:
        parser.add_argument(
            "--solvent",
            default="none",
    ...
    hessian_step = float(getattr(args, "delta_hessian_step", 1.0e-3))
    ...
    solvent_name = getattr(args, "solvent", "none")
    solvent_model = getattr(args, "solvent_model", "alpb")
    if solvent_correction_enabled(solvent_name):
        try:
    ...
    if getattr(args, "embedcharge", False):
        try:
            de_ev, df_local_ev_ang, dh_local_ev_ang2 = delta_embedcharge_minus_noembed(
                ...
                xtb_keep_files=args.xtb_keep_files,
                ncores=resolve_xtb_ncores(),
                hessian_step=getattr(args, "embedcharge_step", 1.0e-3),
            )
        except XTBError as exc:
